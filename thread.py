...
def collect_initial_data():
    cmd = ["nfsiostat", "2", "145"]
    with open('/tmp/nfsiostatdata.txt', 'w') as outputfile:
        subprocess.call(cmd, stdout=outputfile)
        

def format_data(data_type):
    formatted_data = []
    raw_data = getoutput("cat /tmp/nfsiostatdata.txt | awk '{print $%d}'" % COLUMN[data_type]).split("\n")
...
if not os.path.exists("/tmp/nfsiostatdata.txt"):
